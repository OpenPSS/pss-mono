 ...
 * Copyright 2011 Sony Interactive Entertainment America, LLC
 */
...
static void
method_jit_start(MonoProfiler *prof, MonoMethod *method)
{
	uint64_t now;
	char *name;
	int nlen;
	...
	emit_byte (logbuffer, TYPE_JIT_START | TYPE_METHOD);
	emit_time (logbuffer, now);
	emit_method (logbuffer, method);
	memcpy (logbuffer->data, name, nlen);
	logbuffer->data += nlen;
...
mono_profiler_startup (const char *desc)
{
	...
	mono_profiler_install_jit_compile (method_jit_start, NULL);
