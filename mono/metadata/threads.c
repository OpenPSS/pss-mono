...
mono_thread_exit ()
{
	...
	if (mono_thread_get_main () && (thread == mono_thread_get_main ()->internal_thread))
		mono_exit (mono_environment_exitcode_get ());
	ExitThread (-1);
}
							 ...
							 MonoObject *start)
{
	...
	start_func = NULL;
	{
		...
		InterlockedIncrement (&num_threads);
...
suspend_thread_internal (MonoInternalThread *thread, gboolean interrupt)
{
	...
	if (thread == mono_thread_internal_current ()) {
		...
		info = mono_thread_info_safe_suspend_sync ((MonoNativeThreadId)(gsize)thread->tid, interrupt);
		if (!info) {
			LeaveCriticalSection (thread->synch_cs);
			return;
		}
