 ...
 * @exc: The exception
 *
 * Get the unhandled exception message.
 */
char *
mono_unhandled_exception_message (MonoObject *exc)
{
	...
	if (exc == (MonoObject*)mono_object_domain (exc)->out_of_memory_ex) {
		message = (char*) "OutOfMemoryException";
	} else {
	...
	buffer_size = sizeof (message_format) + strlen (message);
	buffer = g_malloc (buffer_size);
	if (buffer) {
		g_snprintf (buffer, buffer_size, message_format, message);
	}
