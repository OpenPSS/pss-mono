...
static guint64
convert_time (uint32_t time)
{
	return ((guint64)time * 10000000) + 116444736000000000ULL;
}
						 ...
						 gint32 *error)
{
	...
	*error = convert_error (res);
	...
	for (i = 0; i < glob_data.gl_pathc; i ++) {
		...
		full_filename = g_build_filename (dirname, name, NULL);
		res = scePssFileGetPathInformation(full_filename, &info);
		if (res != SCE_PSS_ERROR_NO_ERROR                   /* failure to get file info.  shouldn't happen, since this was already done in scePssReadDirectory */
			|| (convert_flags(info.uFlags) & mask) != attrs /* the file doesn't match the attributes we're looking for. */) {
			/* skip it and move on. */
			g_free (full_filename);
			continue;
		}

		g_ptr_array_add (names, full_filename);
	}
				      ...
				      gpointer *handle)
{
	...
	*error = convert_error (res);
	...
	glob_res = _pss_glob (dir, pattern, 0/*FIXME flags*/, &glob_data, scePssDirectoryRead);
	if (glob_res != 0) {
		*error = ERROR_SUCCESS;
		return NULL;
	}
					...
					gint32 *error)
{
	...
	if (res == SCE_PSS_ERROR_NO_ERROR) {
		...
		stat->creation_time = convert_time (info.tCreationTime);
		stat->last_access_time = convert_time (info.tLastAccessTime);
		stat->last_write_time = convert_time (info.tLastWriteTime);
