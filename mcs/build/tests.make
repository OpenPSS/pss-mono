...
## Unit test support
ifndef NO_TEST
test_nunitlite_lib = nunitlite.dll
test_nunitlite_dep = $(test_nunitlite_lib:%=$(topdir)/class/lib/$(PROFILE)/%)
test_nunitlite_ref = $(test_nunitlite_dep:%=-r:%)
tests_CLEAN_FILES += TestResult*.xml

test_lib = $(ASSEMBLY:$(ASSEMBLY_EXT)=_test_$(PROFILE).dll)
test_sourcefile := $(wildcard $(ASSEMBLY:$(ASSEMBLY_EXT)=_test_$(PROFILE).dll.sources))
ifndef test_sourcefile
test_sourcefile := $(ASSEMBLY:$(ASSEMBLY_EXT)=_test.dll.sources)
endif
test_pdb = $(test_lib:.dll=.pdb)
test_response = $(depsdir)/$(test_lib).response
test_makefrag = $(depsdir)/$(test_lib).makefrag
test_flags = -r:$(the_assembly) $(test_nunitlite_ref) $(TEST_MCS_FLAGS)
...
ifndef NO_TEST
$(test_nunitlite_dep): $(topdir)/build/deps/nunitlite-$(PROFILE).stamp
	@if test -f $@; then :; else rm -f $<; $(MAKE) $<; fi
$(topdir)/build/deps/nunitlite-$(PROFILE).stamp:
	cd ${topdir}/nunitlite && $(MAKE)
	echo "stamp" >$@
tests_CLEAN_FILES += $(topdir)/build/deps/nunitlite-$(PROFILE).stamp
endif
...
ifeq ($(PROFILE),mobile)
PROFILE_EXCLUSIONS =,NotMobile,PssFileIO
endif

TEST_HARNESS_EXCLUDES = -exclude=$(PLATFORM_TEST_HARNESS_EXCLUDES)NotWorking,ValueAdd,CAS,InetAccess$(PROFILE_EXCLUSIONS)
...
$(test_lib): $(the_assembly) $(test_response) $(test_nunitlite_dep)
